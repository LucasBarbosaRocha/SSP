...
#define maxQTDA 8 // Duas linhas por sequencia
#define maxQTDB 2000
...
int main(int argc, char **argv)
{
    ...
    int err[maxError], errAux[maxError];
    time_t inicio, fim;
    MPI_Status status;
    char subVetorA[maxSubVetor], subVetorB[maxSubVetor], sequenciaChar[maxSubVetor];
    ...
    if (rank == 0) 
    {
        ...
        int qtdProcessadorA = maxQTDA / (size - 1);
        /* Arquivo resposta */
        fstream out;
        out.open("Saidas/ERR599040-P1-A-10", ios::out | ios::trunc);        
        ...
        while(controlaA < maxQTDA)
        {
            ...
            while(controlaB < maxQTDB)
            {
                ...
                for (int i = 1; i < size; i++){
                    ...
                    for (int k = 0; k < (qtdProcessadorA / 2); k++)
                    {
                        rc = MPI_Recv(errAux, maxError, MPI_INT, i, tag, MPI_COMM_WORLD, &status);
                        printf("Recebi %d Escrevendo sequencia %d maximo %d \n", i, nSequence, qtdProcessadorA);
        ...
        arquivoA.close();
        out.close(); // Fechando o arquivo resposta
        time(&fim);
